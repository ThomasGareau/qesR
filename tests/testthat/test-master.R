...
test_that("get_qes_master removes duplicate respondents within the same year", {
  local_mocked_bindings(
    get_qes = function(srvy, assign_global, with_codebook, quiet) {
  ...
  expect_true(identical(sum(master$respondent_id == "12"), 1L))
  expect_true(identical(master$age[master$respondent_id == "12"], 40))
})
