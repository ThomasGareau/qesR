---
title: "Exemple de cas d'usage : statistiques descriptives"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Exemple de cas d'usage : statistiques descriptives}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  echo = FALSE,
  warning = FALSE,
  message = FALSE
)
```

```{r}
library(qesR)
library(dplyr)
library(ggplot2)
library(knitr)
```

```{r}
pkg_master <- system.file("extdata", "qes_master.csv", package = "qesR")
master_paths <- c("qes_master.csv", "../qes_master.csv", pkg_master)
master_paths <- master_paths[nzchar(master_paths)]
master_path <- master_paths[file.exists(master_paths)][1]
if (is.na(master_path) || !nzchar(master_path)) {
  stop("qes_master.csv introuvable localement. Lancez d'abord scripts/build_qes_master.R", call. = FALSE)
}
master <- read.csv(master_path, stringsAsFactors = FALSE)

parser_turnout <- function(x) {
  brut <- trimws(as.character(x))
  norm <- tolower(iconv(brut, from = "", to = "ASCII//TRANSLIT"))
  out <- rep(NA_real_, length(norm))

  as_num <- suppressWarnings(as.numeric(brut))
  is_num <- is.finite(as_num)
  if (any(is_num)) {
    uniq <- unique(as_num[is_num])
    if (all(uniq %in% c(0, 1))) {
      out[is_num] <- as_num[is_num]
    }
  }

  non_hit <- grepl("^(0|2|non|no)$|n[' ]?a pas vot|did not vot|didn[' ]?t vot|annule|ne votera pas", norm, perl = TRUE)
  oui_hit <- grepl("^(1|oui|yes)$|\\boui\\b|\\byes\\b|already voted|par anticipation|le jour meme|certain to vote", norm, perl = TRUE)

  out[non_hit] <- 0
  out[oui_hit & !non_hit] <- 1
  out
}

collapse_age_group3 <- function(x) {
  brut <- trimws(as.character(x))
  brut[!nzchar(brut)] <- NA_character_
  norm <- tolower(iconv(brut, from = "", to = "ASCII//TRANSLIT"))
  out <- rep(NA_character_, length(norm))

  out[grepl("18\\s*[-toa ]\\s*24|25\\s*[-toa ]\\s*34|18\\s*[-toa ]\\s*34|de\\s*18\\s*a\\s*24|de\\s*25\\s*a\\s*34", norm, perl = TRUE)] <- "18-34"
  out[grepl("35\\s*[-toa ]\\s*44|45\\s*[-toa ]\\s*54|35\\s*[-toa ]\\s*54|de\\s*35\\s*a\\s*44|de\\s*45\\s*a\\s*54", norm, perl = TRUE)] <- "35-54"
  out[grepl("55\\s*[-toa ]\\s*64|65\\+|65\\s*ans\\s*et\\s*plus|55\\+|55\\s*ans\\s*et\\s*plus", norm, perl = TRUE)] <- "55+"

  out
}

master <- master %>%
  mutate(
    annee = suppressWarnings(as.integer(substr(as.character(qes_year), 1, 4))),
    age_group = trimws(as.character(age_group)),
    turnout_num = parser_turnout(turnout)
  )

# qes_crop_2007_2010 est un fichier regroupé sans variable d'année par ligne.
# On le garde dans les tableaux par étude, mais pas dans les tendances annuelles.
master_tendance <- master %>%
  filter(qes_code != "qes_crop_2007_2010")
```

## Tableau 1 : taille des échantillons

```{r}
tailles <- master %>%
  group_by(qes_year, qes_code, qes_name_en) %>%
  summarise(n = n(), .groups = "drop") %>%
  arrange(qes_year, qes_code)

knitr::kable(
  tailles %>%
    transmute(
      `Année d'étude` = qes_year,
      `Code étude` = qes_code,
      `Taille échantillon` = n
    ),
  caption = "Taille d'échantillon par étude"
)
```

## Tableau 2 : répartition des groupes d'âge

```{r}
ordre_age <- c("18-34", "35-54", "55+")

age_dist <- master_tendance %>%
  mutate(age_group_3 = collapse_age_group3(age_group)) %>%
  filter(!is.na(annee), !is.na(age_group_3), nzchar(age_group_3)) %>%
  group_by(annee, age_group_3) %>%
  summarise(n = n(), .groups = "drop_last") %>%
  mutate(pct = 100 * n / sum(n)) %>%
  ungroup() %>%
  arrange(annee, age_group_3)

age_dist$age_group_3 <- factor(age_dist$age_group_3, levels = ordre_age)

age_resume <- age_dist %>%
  group_by(annee) %>%
  summarise(
    n_avec_age = sum(n),
    pct_18_34 = ifelse(
      n_avec_age > 0,
      100 * sum(n[age_group_3 == "18-34"], na.rm = TRUE) / n_avec_age,
      NA_real_
    ),
    pct_35_54 = ifelse(
      n_avec_age > 0,
      100 * sum(n[age_group_3 == "35-54"], na.rm = TRUE) / n_avec_age,
      NA_real_
    ),
    pct_55_plus = ifelse(
      n_avec_age > 0,
      100 * sum(n[age_group_3 == "55+"], na.rm = TRUE) / n_avec_age,
      NA_real_
    ),
    .groups = "drop"
  ) %>%
  arrange(annee)

knitr::kable(
  age_resume %>%
    transmute(
      `Année d'étude` = annee,
      `N avec groupe d'âge` = n_avec_age,
      `18-34 (%)` = round(pct_18_34, 1),
      `35-54 (%)` = round(pct_35_54, 1),
      `55+ (%)` = round(pct_55_plus, 1)
    ),
  digits = 1,
  caption = "Répartition des groupes d'âge par année"
)
```

## Figure 1 : profil des groupes d'âge dans le temps

```{r, fig.alt="Graphique en lignes de la répartition harmonisée des groupes d'âge selon l'année."}
age_dist$age_group_3 <- factor(age_dist$age_group_3, levels = ordre_age)
ymax_age <- suppressWarnings(max(age_dist$pct, na.rm = TRUE))
ymax_age <- if (is.finite(ymax_age)) min(100, ymax_age + 5) else 100

ggplot(age_dist, aes(x = annee, y = pct, color = age_group_3, group = age_group_3)) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 2.2) +
  geom_smooth(
    method = "loess",
    se = FALSE,
    span = 0.9,
    linewidth = 0.8,
    linetype = "dashed"
  ) +
  scale_color_manual(
    values = c(
      "18-34" = "#2c7fb8",
      "35-54" = "#12355b",
      "55+" = "#b5651d"
    ),
    name = "Groupe d'âge"
  ) +
  scale_y_continuous(labels = function(x) paste0(round(x, 0), "%"), limits = c(0, ymax_age)) +
  scale_x_continuous(breaks = sort(unique(age_dist$annee))) +
  labs(
    title = "Répartition des groupes d'âge dans le temps",
    x = "Année d'étude",
    y = "Part des répondants"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor = element_blank(),
    axis.text.x = element_text(size = 9)
  )
```

## Tableau 3 : participation électorale par année

```{r}
annees <- data.frame(annee = sort(unique(master_tendance$annee[!is.na(master_tendance$annee)])))

turnout_annee <- master_tendance %>%
  filter(!is.na(annee)) %>%
  group_by(annee) %>%
  summarise(
    repondants = n(),
    n_avec_turnout = sum(!is.na(turnout_num)),
    taux = ifelse(n_avec_turnout > 0, mean(turnout_num, na.rm = TRUE), NA_real_),
    se = ifelse(
      n_avec_turnout > 0,
      sqrt(pmax(taux * (1 - taux), 0) / n_avec_turnout),
      NA_real_
    ),
    ci_bas = ifelse(!is.na(se), pmax(0, taux - 1.96 * se), NA_real_),
    ci_haut = ifelse(!is.na(se), pmin(1, taux + 1.96 * se), NA_real_),
    .groups = "drop"
  )

turnout_annee <- merge(annees, turnout_annee, by = "annee", all.x = TRUE, sort = TRUE)

knitr::kable(
  turnout_annee %>%
    mutate(
      taux_pct = round(100 * taux, 1),
      ci_bas_pct = round(100 * ci_bas, 1),
      ci_haut_pct = round(100 * ci_haut, 1)
    ) %>%
    transmute(
      `Année d'étude` = annee,
      `N répondants` = repondants,
      `N avec item participation` = n_avec_turnout,
      `Taux de participation (%)` = taux_pct,
      `IC 95 % (%)` = ifelse(is.na(ci_bas_pct), NA_character_, sprintf("%.1f à %.1f", ci_bas_pct, ci_haut_pct))
    ),
  caption = "Indicateur de participation par année"
)
```

## Figure 2 : tendance de la participation électorale

```{r, fig.alt="Graphique de l'indicateur de participation électorale selon l'année."}
plot_turnout <- turnout_annee %>%
  filter(!is.na(taux), !is.na(ci_bas), !is.na(ci_haut))
ymax_turnout <- suppressWarnings(max(plot_turnout$ci_haut, plot_turnout$taux, na.rm = TRUE))
ymax_turnout <- if (is.finite(ymax_turnout)) min(1, ymax_turnout + 0.05) else 1

ggplot(plot_turnout, aes(x = annee, y = taux)) +
  geom_ribbon(aes(ymin = ci_bas, ymax = ci_haut), fill = "#9fb6ce", alpha = 0.35, na.rm = TRUE) +
  geom_line(linewidth = 1, color = "#12355b", na.rm = TRUE) +
  geom_point(size = 2.3, color = "#b5651d", na.rm = TRUE) +
  geom_smooth(
    method = "loess",
    se = TRUE,
    span = 0.9,
    color = "#12355b",
    fill = "#c9d6e4",
    alpha = 0.25,
    linetype = "dashed",
    linewidth = 0.8,
    na.rm = TRUE
  ) +
  scale_y_continuous(labels = function(x) paste0(round(100 * x, 0), "%"), limits = c(0, ymax_turnout)) +
  scale_x_continuous(breaks = sort(unique(plot_turnout$annee))) +
  labs(
    title = "Indicateur de participation électorale",
    x = "Année d'étude",
    y = "Part des répondants"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor = element_blank(),
    axis.text.x = element_text(size = 9)
  )
```

## Notes

- `qes_crop_2007_2010` est exclu des tableaux et graphiques par année parce que
  ce fichier ne contient pas d'année au niveau des répondants.
