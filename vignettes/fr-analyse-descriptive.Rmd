...
age_counts <- age_dist %>%
  group_by(annee) %>%
  summarise(n_avec_age = sum(n), .groups = "drop")

age_dist_large <- reshape(
  age_dist[, c("annee", "age_group_3", "pct")],
  idvar = "annee",
  timevar = "age_group_3",
  direction = "wide"
)

names(age_dist_large) <- sub("^pct\\.", "", names(age_dist_large))

for (grp in ordre_age) {
  if (!(grp %in% names(age_dist_large))) {
    age_dist_large[[grp]] <- NA_real_
  }
}

age_dist_large <- age_dist_large %>%
  merge(age_counts, by = "annee", all.x = TRUE, sort = TRUE) %>%
  arrange(annee)
