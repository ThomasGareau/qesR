---
title: "Use case example: Descriptive Statistics"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Use case example: Descriptive Statistics}
...
master <- master %>%
  mutate(
    qes_year_num = suppressWarnings(as.integer(substr(as.character(qes_year), 1, 4))),
...
age_table$age_group_3 <- factor(age_table$age_group_3, levels = age_order)
ymax_age <- suppressWarnings(max(age_table$pct, na.rm = TRUE))
...
ggplot(age_table, aes(x = qes_year_num, y = pct, color = age_group_3, group = age_group_3)) +
  ...
  scale_x_continuous(breaks = sort(unique(age_table$qes_year_num))) +
  labs(
...
ymax_turnout <- suppressWarnings(max(plot_turnout$ci_high, plot_turnout$turnout_rate, na.rm = TRUE))
...
ggplot(plot_turnout, aes(x = qes_year_num, y = turnout_rate)) +
  ...
  scale_x_continuous(breaks = sort(unique(plot_turnout$qes_year_num))) +
  labs(
