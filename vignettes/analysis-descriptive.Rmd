...
age_counts <- age_table %>%
  group_by(qes_year_num) %>%
  summarise(n_with_age_group = sum(n), .groups = "drop")

age_table_wide <- reshape(
  age_table[, c("qes_year_num", "age_group_3", "pct")],
  idvar = "qes_year_num",
  timevar = "age_group_3",
  direction = "wide"
)

names(age_table_wide) <- sub("^pct\\.", "", names(age_table_wide))

for (grp in age_order) {
  if (!(grp %in% names(age_table_wide))) {
    age_table_wide[[grp]] <- NA_real_
  }
}

age_table_wide <- age_table_wide %>%
  merge(age_counts, by = "qes_year_num", all.x = TRUE, sort = TRUE) %>%
